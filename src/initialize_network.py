...
# plt.scatter(x2,y2,c='.75',alpha=.25)
# labels = list(deg_people.keys())
people_nodes = [n for n,d in B.nodes(data=True) if d['bipartite'] == 1]
x = [deg_people[l] for l in people_nodes]
y = [betw[l] for l in people_nodes]

plt.scatter(x,y,c='r',alpha=1)

for label, a, b in zip(people_nodes,x,y):
    if a > 4:
        plt.annotate(
            label,
            xy=(a,b), xytext=(-1,1),
